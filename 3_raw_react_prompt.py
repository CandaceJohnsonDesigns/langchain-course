...
# CHANGE 3: Delete the JSON schemas. Tools now live inside the prompt as plain text. We derive descriptions from the functions themselves using inspect.
...
# CHANGE 4: Drop tools= from ollama.chat(). The LLM has no idea it's an agent - all agency comes from the prompt above and our regex parsing below.
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # CHANGE 6: Parse tool calls from raw text with regex - fragile if LLM doesn't follow the format exactly.
